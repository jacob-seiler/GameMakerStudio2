...
// TODO use the step event for some of this, not the draw gui event!
...
if (!done) {
	...
	if (script_control_check_pressed(controls.start)) {
		done = true;
	}
} else {
	if (fixed) {
		...
		} else if (script_control_check_pressed(controls.a) || script_control_check_pressed(controls.x)) {
			// TODO advance
